...
@implementation jsOAuthTestSuiteObject
...
-(void)dealloc
{
	...
	RELEASE_TO_NIL(_jsoauthtestsuite);
	RELEASE_TO_NIL(modules);
...
-(void)injectPatches
{
	// called to inject any jsOAuthTestSuite patches in JS before a context is loaded... nice for 
...
{
	// create jsOAuthTestSuite global object
	NSString *basePath = (url==nil) ? [[NSBundle mainBundle] resourcePath] : [[url path] stringByDeletingLastPathComponent];
	_jsoauthtestsuite = [[jsOAuthTestSuiteObject alloc] initWithContext:kroll host:host context:self baseURL:[NSURL fileURLWithPath:basePath]];
	
	TiContextRef jsContext = [kroll context];
	TiValueRef tiRef = [KrollObject toValue:kroll value:_jsoauthtestsuite];
	
	NSString *_jsoauthtestsuiteNS = [NSString stringWithFormat:@"T%sanium","it"];
	TiStringRef prop = TiStringCreateWithUTF8CString([_jsoauthtestsuiteNS UTF8String]);
	...
	if (preload!=nil)
	{
		KrollObject *ti = (KrollObject*)[_jsoauthtestsuite valueForKey:@"UI"];
...
-(id)require:(KrollContext*)kroll path:(NSString*)path
{
	...
	@throw [NSException exceptionWithName:@"org.jsoauthtestsuite.kroll" reason:[NSString stringWithFormat:@"Couldn't find module: %@",path] userInfo:nil];
}
