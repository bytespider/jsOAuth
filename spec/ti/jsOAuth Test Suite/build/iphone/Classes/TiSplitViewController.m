...
{
	if (self = [super init]) {
		_jsoauthtestsuiteRoot = [rootController retain];
		proxy = split_;
...
-(void)dealloc
{
	RELEASE_TO_NIL(_jsoauthtestsuiteRoot);
...
-(void)loadView
{
	[[NSNotificationCenter defaultCenter] removeObserver:_jsoauthtestsuiteRoot name:UIDeviceOrientationDidChangeNotification object:nil];
	[super loadView];
}
...
{
	[_jsoauthtestsuiteRoot refreshOrientationModesIfNeeded:oldCurrentWindow];
}
